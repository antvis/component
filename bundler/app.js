...
const assign = require('@antv/util').mix;
...
function startService(port) {
      ...
      if (req.method === 'GET') {
        ...
        if (pathname === '/bundle') {
          ...
          webpack(config, (err, stats) => {
            ...
            // step3: uglify
            shelljs.exec('uglifyjs -c -m -o __dist/component.min.js -- __dist/component.js');
            // step4: zipping it
            const zip = new JSZip();
            zip.folder('component-dist').file('entry.js', entryFileContent);
            zip.folder('component-dist').file('component.js', readFileSync(join(distPath, './component.js'), 'utf8'));
            zip.folder('component-dist').file('component.js.map', readFileSync(join(distPath, './component.js.map'), 'utf8'));
            zip.folder('component-dist').file('component.min.js', readFileSync(join(distPath, './component.min.js'), 'utf8'));
            res.writeHead(200, {
  ...
  if (commander.web) {
    ...
    const windowBoundsConfig = require('torchjs/lib/windowBoundsConfig')(
      resolve(app.getPath('userData'), './component-bundler-config.json')
    );
