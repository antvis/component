...
export const AxisLabelRotateUpdate = () => {
  ...
  const style1 = {
    ...
    labelTransform: [{ type: 'rotate', optionalAngles: [0, 10, 20, 30, 40, 45, 60, 90] }],
    showLabel: true,
    showGrid: false,
    showTick: true,
    showLine: true,
  ...
  const update = (animate: any = undefined) => {
    axis.update({ style: { endPos: [800, 50] } }, animate);
  };

  const reset = (animate: any = undefined) => {
    axis.update({ style: { endPos: [600, 50] } }, animate);
  };

  group.appendChild(new Button({ style: { style: { text: 'update', onClick: () => update() } } }));

  group.appendChild(new Button({ style: { style: { x: 80, text: 'reset', onClick: () => reset() } } }));
