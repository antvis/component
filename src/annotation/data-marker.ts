...
class DataMarkerAnnotation extends GroupComponent<DataMarkerAnnotationCfg> implements ILocation<PointLocationCfg> {
  ...
  private autoAdjust(group: IGroup) {
    ...
    if (
      ...
      (direction === 'downward' && y + maxY >= coordinateBBox.maxY)
    ) {
      // 上方或者下方超出
      let textBaseline;
      let factor;
    ...
    if (
      (direction === 'leftward' && x + minX <= coordinateBBox.minX) ||
      (direction === 'rightward' && x + maxX >= coordinateBBox.maxX)
    ) {
      // 左侧或者右侧超出
      let textAlign;
      let factor;
      if (direction === 'leftward' && x + minX <= coordinateBBox.minX) {
        textAlign = 'start';
        factor = 1;
      } else {
        textAlign = 'end';
      ...
      if (lineShape) {
        lineShape.attr('path', [
          ['M', 0, 0],
          ['L', lineLength * factor, 0],
        ]);
      }
      applyTranslate(textGroup, (lineLength + 2) * factor, textGroup.attr('y'))
    }
  }
  ...
  private getShapeAttrs() {
    ...
    let textBaseline = 'top', textAlign = 'start';
    switch (direction) {
      case 'upward': 
        yFactor = -1;
        textBaseline = 'bottom';
